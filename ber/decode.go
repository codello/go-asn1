...
// implement the [BerMatcher] interface to provide support for optional
// elements.
//
// The reader r reads the contents of the element, identified by tag. In
// particular r does not read the tag and length bytes of the element being
// decoded. At the end of the element r returns io.EOF, even if there are more
// bytes in the original data stream. If an implementation does not read r to
// completion, any remaining bytes are discarded. If r.Constructed() returns
// true, the remaining elements are syntactically validated.
//
...
// requirements of the element. If decoding fails, an error must be returned
// that explains the failure. Usually such an error should be a [SyntaxError] or
// [StructuralError]. Implementations can also return [io.ErrUnexpectedEOF] to
// indicate that r returned io.EOF before the entire data was read.
//
// The length of the element being decoded is available through r. Note that an
// element may use the indefinite length encoding in which case r might indicate
// a length of [LengthIndefinite].
...
// support for optional elements. The BerMatch method is consulted if no tag
// number is given via struct tags. Implementations implement this interface by
// returning a boolean value indicating whether h.Class and h.Tag match the
// intrinsic tag of the element, i.e. if based on the h.Class and h.Tag it is
...
// indicate that the nesting of structured elements contains an error, or that a
// primitive element could not be decoded from its BER representation.
//
...
//region types ElementReader and elementReader

// ElementReader is a reader type for reading a BER-encoded element.
//
// Elements can use the primitive or constructed encoding method. Depending on
// the encoding method only certain methods can be used. Reading methods of an
// ElementReader return io.EOF when the element is read to completion, although
// the underlying data source might have more bytes available. Before the
// element is read to completion, io.ErrUnexpectedEOF may be returned.
//
// Closing an ElementReader is optional. Closing validates that the remaining
// bytes of a constructed element are valid BER-encoding.
...
// elementReader is the primary implementation of ElementReader in this package.
// An elementReader can operate in two modes (primitive or constructed)
// indicated by H. Switching between the two modes is not supported.
type elementReader struct {
	H Header
	R *limitReader // underlying reader

	// curr is the reader for the last child element returned by Next.
	curr *elementReader
	...
	// root indicates that Next() may return io.EOF if the underlying reader returns
	// io.EOF at the start of an element.
	root bool
}

// Constructed reports whether r is operating on a constructed or primitive
// element.
func (r *elementReader) Constructed() bool {
	return r.H.Constructed
}

// More indicates whether there might be more data in d that can be decoded.
//
// If r encounters a BER-encoded type during decoding that is syntactically
// invalid, r tries to discard the respective element so that decoding can
// continue. However, some errors are irrecoverable so that r is in an unsafe
// state where decoding more elements may yield invalid results.
//
// This method indicates whether r is in a valid state and can decode more
// elements. This method does not indicate if there are more elements. In
...
// return false.
func (r *elementReader) More() bool {
...
func (r *elementReader) Len() int {
	return r.R.Len()
}

// Next parses the next element from r. This method implements [ElementReader],
// see [ElementReader.Next] for details. If r is not constructed, an error will
// be returned.
//
// The returned [ElementReader] is valid until the next call of Next. The caller
// of this method is responsible for closing the returned [ElementReader] in
...
func (r *elementReader) Next() (h Header, er ElementReader, err error) {
	if !r.Constructed() {
		return Header{}, nil, &SyntaxError{r.H.Tag, errors.New("primitive element")}
	}
	...
	if err != nil {
		...
		// We cannot know that the following bytes are the start of a new element.
		r.err = err
		return Header{}, nil, r.err
		...
		r.err = &SyntaxError{r.H.Tag, fmt.Errorf("primitive element %s has indefinite length", h.Tag.String())}
		return Header{}, nil, r.err
	}
	// If we reach this point, the header is syntactically valid. All the following
	// errors are non-fatal as we might be able to discard the element successfully.

	if h == (Header{}) {
		err = &SyntaxError{r.H.Tag, errors.New("unexpected end of contents")}
	} else if h.Tag == (asn1.Tag{Class: asn1.ClassUniversal, Number: asn1.TagReserved}) && (h.Constructed || h.Length != 0) {
	...
	if h.Length == LengthIndefinite {
		...
		// elements inside indefinite-length elements exceed a top-level definite-length
		// element.
		lr.N = r.R.Len()
	} else if r.R.Limited() && h.Length > r.R.Len() {
		// We return the reader for the element as the element might still be readable.
		// We do not adjust lr.Len() in order to trigger an ErrUnexpectedEOF when
		// reading the element.
		err = &SyntaxError{r.H.Tag, fmt.Errorf("child element %s exceeds its parent", h.Tag.String())}
	}
	r.curr = &elementReader{H: h, R: lr}
	return h, r.curr, err
}

// Close closes r. If r is primitive any unread bytes are discarded. If r is
// constructed this recursively validates that any unread elements of r are
// syntactically valid. If a syntax error is encountered, it is returned and
// validation stops.
func (r *elementReader) Close() (err error) {
	if !r.Constructed() {
...
// the unread bytes are simply discarded. If r uses the indefinite-length method
// r.Next is called (which recursively discards unprocessed elements) until
...
// elements or data.
func (r *elementReader) discard() (err error) {
...
// Read implements the io.Reader interface. If r is constructed, this method
// returns an error.
func (r *elementReader) Read(p []byte) (n int, err error) {
	if r.Constructed() {
		return 0, fmt.Errorf("element %s is constructed, expected primitive", r.H.Tag.String())
	}
	if r.err != nil {
		return 0, r.err
	}
...
// ReadByte implements the io.ByteReader interface. If r is constructed, this
// method returns an error.
func (r *elementReader) ReadByte() (byte, error) {
	if r.Constructed() {
		return 0, fmt.Errorf("element %s is constructed, expected primitive", r.H.Tag.String())
	}
	if r.err != nil {
...
// decoderReader wraps a [Decoder] to implement the [ElementReader] interface.
...
// explicitDecoder implements decoding of ASN.1 EXPLICIT elements. Explicit
// elements are wrapped in another constructed element with a unique tag. The
// tag is defined by struct tags so explicitDecoder does not have an intrinsic
// tag and does not implement [BerMatcher].
type explicitDecoder codec[BerDecoder]

// BerDecode decodes a single element from r into d.dec.
func (d *explicitDecoder) BerDecode(tag asn1.Tag, r ElementReader) (err error) {
	if r.Len() == 0 {
		...
		return &SyntaxError{tag, errors.New("non-constructed explicit element")}
	}
	h, er, err := r.Next()
	if err != nil {
		return err
	...
	_, _, err = r.Next()
	if err == nil {
		return &SyntaxError{tag, errors.New("explicit element has multiple children")}
	}
	if err != io.EOF {
		return err
	}
...
// cannot be filled completely or there are additional elements, an error is
// generated.
...
func (d sequenceDecoder) BerMatch(tag asn1.Tag) bool {
	...
	return tag == asn1.Tag{Class: asn1.ClassUniversal, Number: asn1.TagSequence} ||
		tag == asn1.Tag{Class: asn1.ClassUniversal, Number: asn1.TagSet}
}

// BerDecode parses a BER-encoded sequence of elements into d.Value. If d.Value
// is a slice it is resized according to the number of elements found.
func (d sequenceDecoder) BerDecode(tag asn1.Tag, r ElementReader) (err error) {
	seqType := d.ref.Type()
	...
	for i = 0; err == nil && (d.ref.Kind() != reflect.Array || i < d.ref.Len()); i++ {
		...
		// allocate a new zero element that is addressable
	...
	for ; err == nil; i++ {
		// read all extra elements until we hit an error
	...
	i-- // the last EOF is not an element read
	if i > d.ref.Len() {
...
func (d structDecoder) BerDecode(tag asn1.Tag, r ElementReader) error {
	h, er, err := r.Next()
	for field, params := range internal.StructFields(d.ref) {
		...
		if field.Type() == internal.ExtensibleType {
			// read and validate all remaining elements
			err = er.Close()
			for err == nil {
				if _, er, err = r.Next(); err == nil {
					err = er.Close()
		...
		if errors.Is(err, errElementMismatch) && params.Optional {
			err = nil
			continue
		}
		return err
...
//region general decoder selection via reflection

// errElementMismatch is a sentinel error returned by decodeValue that indicates
// that the element did not match the provided header. This error is used to
// implement optional elements.
var errElementMismatch = errors.New("elements do not match")
...
// errElementMismatch is returned. If no decoder is available for v, decodeValue
// returns an InvalidDecodeError.
func decodeValue(tag asn1.Tag, r ElementReader, v reflect.Value, params internal.FieldParameters) error {
...
// errElementMismatch is returned. If no decoder is available for v, makeDecoder
// returns an InvalidDecodeError.
func makeDecoder(tag asn1.Tag, v reflect.Value, params internal.FieldParameters) (ret BerDecoder, err error) {
	if params.Nullable && (tag == asn1.Tag{Class: asn1.ClassUniversal, Number: asn1.TagNull}) {
		return nullCodec{ref: v}, nil
	}

	// we have an explicitly set tag. ignore the intrinsic type match
	if params.Tag != nil && tag != *params.Tag {
		return nil, &StructuralError{tag, v.Type(), fmt.Errorf("explicit element %s: %w", params.Tag.String(), errElementMismatch)}
	}
	...
	defer func() {
		...
		if params.Tag == nil && v.Kind() != reflect.Interface {
			if m, ok := ret.(BerMatcher); ok && !m.BerMatch(tag) {
				ret = nil
				err = &StructuralError{tag, v.Type(), errElementMismatch}
				return
			}
		}
		if params.Explicit {
	...
	for v.Kind() == reflect.Interface || v.Kind() == reflect.Pointer {
		...
		if v.IsNil() {
			...
			// the value immediately because we don't know yet if the elements match.
			if field.IsValid() {
...
// bytes than required to parse one element.
//
// If r implements [ElementReader] and is reading a constructed element, d will
// decode those elements from r without additional buffering.
//
...
// format on the top-level element, d will not read more bytes from r than
// required to parse one element. If the indefinite-length encoding is used,
// then d might read more bytes from r than needed.
func NewDecoder(r io.Reader) (d *Decoder) {
	if er, ok := r.(ElementReader); ok && er.Constructed() {
		return &Decoder{er: er}
	}
	er := &elementReader{
...
//
// If d encounters a BER-encoded type during decoding that is syntactically
// invalid, d tries to discard the respective element so that decoding can
// continue. However, some errors are irrecoverable so that d is in an unsafe
// state where decoding more elements may yield invalid results.
//
// This method indicates whether d is in a valid state and can decode more
// elements. This method does not indicate if there are more elements. In
// particular a return value of true does not guarantee that d.Next() does not
// return an error.
//
...
//
// The returned ElementReader is valid until the next call to Next(). If the
...
// returned ElementReader in order to validate the syntax of any remaining
// bytes.
//
// If no more elements are available, io.EOF is returned.
func (d *Decoder) Next() (Header, ElementReader, error) {
	...
	if er != nil && d.buf != nil {
		...
		if h.Length == LengthIndefinite {
			...
			//  - The element was discarded during d.er.Next(). Then d.buf.Buffered() is 0
			//    and we reset the buffer below.
			//  - The element was not discarded due to an error. Then d.er.Next() will not
			//    return an element so we won't get here.
...
// parameters applied to the top-level element. The format for params is the
// same as for struct tags supported by this package. Using the
...
// DecodeAll decodes all elements from d into the value pointed to by val. The
...
func (d *Decoder) DecodeAll(val any) error {
	...
	return decodeValue(asn1.Tag{Class: asn1.ClassUniversal, Number: asn1.TagSequence}, &decoderReader{d}, v.Elem(), internal.FieldParameters{})
}
...
// element. The form of the params is the same as the field tags. See
// [Decoder.Decode] for details.
func UnmarshalWithParams(b []byte, val any, params string) error {
	r := bytes.NewReader(b)
	d := NewDecoder(r)
	err := d.DecodeWithParams(val, params)
	if err == nil && r.Len() > 0 {
		return errors.New("extra data after element")
	}
	return err
}
